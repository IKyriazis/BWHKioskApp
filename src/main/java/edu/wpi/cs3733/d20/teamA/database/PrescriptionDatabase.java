...
public class PrescriptionDatabase extends Database{
    private int prescriptionNum = getSizePrescription() + 1;
    ...
    protected boolean createTables(){
        return helperPrepared("CREATE TABLE PRESCRIPTION(prescriptionID INTEGER PRIMARY KEY, patientName VARCHAR(50) NOT NULL, prescription VARCHAR(50) NOT NULL, dosage VARCHAR(25), numberOfRefills INTEGER NOT NULL, refillPer VARCHAR(20), doctorLastName VARCHAR(25) NOT NULL, notes VARCHAR(100), CONSTRAINT FK_DOCTOR FOREIGN KEY (doctorLastName) REFERENCES Employees(nameLast), CONSTRAINT CH_PER CHECK( refillPer in ('DAY', 'WEEK', 'MONTH','YEAR')), CONSTRAINT CH_NUMREFILL CHECK(numberOfRefills > 0))");
    }
    ...
    protected boolean addPrescription(int prescriptionNum, String patient,
                                      String dosage, int numRefills,
                                      String refillPer, String doctorName, String notes){
        return false;
    }
