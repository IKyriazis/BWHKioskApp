...
public class InventoryDatabase extends Database implements IDatabase{
    ...
    public synchronized boolean createTables() {
        if(doesTableNotExist("INVENTORY")) {
            return helperPrepared("CREATE TABLE Inventory (itemID Varchar(6) PRIMARY KEY, " +
    ...
    public synchronized String addItem(String type, String name, int quantity, double unitPrice, String description, String additional) {
        ...
        try {
            if(a) {
                PreparedStatement pstmt = getConnection().prepareStatement("INSERT INTO Inventory (itemID, itemType, itemName, quantity, unitPrice, description, additional) VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    public synchronized boolean removeItem(String itemID) {
        try {
            if(checkIfExistsString("Inventory", "itemID", itemID)) {
                PreparedStatement pstmt = getConnection().prepareStatement("DELETE FROM Inventory WHERE itemID = ?");
                pstmt.setString(1, itemID);
                pstmt.executeUpdate();
                pstmt.close();
                return true;
