...
public class LaundryDatabase extends Database {
    ...
    public boolean dropTables() {
        boolean a = helperPrepared("DROP TABLE Laundry");
        return a;
    }
    ...
    public boolean addLaundry(String emp, String loc) {
        try {
            PreparedStatement pstmt = getConnection().prepareStatement("INSERT INTO Laundry (requestNum, employeeEntered, location, progress) VALUES (?, ?, ?, 'Request Sent')");
    ...
    public boolean editLaundry(int num, String emp, String loc, String prog, String empW) {
        try {
            PreparedStatement pstmt = getConnection().prepareStatement("UPDATE Laundry SET employee = ?, location = ?, progress = ?, employeeWash = ? WHERE requestNum = ?");
    ...
    public boolean deleteLaundry(int num) {
        try {
            PreparedStatement pstmt = getConnection().prepareStatement("DELETE FROM Laundry WHERE requestNum = ?");
            pstmt.setInt(1, num);
            pstmt.executeUpdate();
            pstmt.close();
            return true;
    ...
    public boolean helperSetString(int num, String col, String s) {
        try {
            PreparedStatement pstmt = getConnection().prepareStatement("UPDATE Laundry SET " + col + " = " + s + " WHERE requestNum = " + num);
            pstmt.executeUpdate();
            pstmt.close();
            return true;
        } catch (SQLException e) {
